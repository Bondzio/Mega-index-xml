...
// 				// foreach array 0 and array 1;
						foreach ($duplicate_arr[$key] as $tmp_glob) {
							// pre_print_r(array_keys($duplicate_arr[$key]));
							// [0] => 0
							// [1] => 1
							if(array_key_exists('unterBegriff', $tmp_glob)){
								foreach ($tmp_glob['unterBegriff'] as $unter_k => $unter_v) {
									...
									if($unter_v['uname'] == $v){
										if (isset($unter_v['group']['entry'])) {
											$tmp_value = array($unter_v['group']['entry']);
										}else{
											$tmp_value = $unter_v['group'];
										}
										// pre_print_r(array_keys($tmp_value));
										foreach ($tmp_value as $k_unter_v => $v_unter_v) {
											// pre_print_r(array_keys($v_unter_v));
											if(!array_key_exists('entry', $v_unter_v)){
												if(isset($v_unter_v[0])){
													$v_unter_v = array('entry' => array_values($v_unter_v));
												}else{
													$v_unter_v = array('entry' => array($v_unter_v));
												}

											}else{
												if(!isset($v_unter_v['entry'][0])){
													$v_unter_v['entry'] = array($v_unter_v['entry']);
												}
											}
											$o_u_duplicate_list_id[$key]['unterBegriff'][$k]['group'][] = $v_unter_v;
											// pre_print_r(array_keys($o_u_duplicate_list_id[$key]['unterBegriff'][$k]['group']));
										}
									}						
								}
							}
...
// pre_print_r($o_u_duplicate_list_id['A0023']);
// pre_print_r($o_u_duplicate_list_id['A0023']['unterBegriff']['A0023_041']['group']);
// pre_print_r($o_u_duplicate_list_id['A0023']['unterBegriff']['A0023_041']);
	// pre_print_r($o_u_duplicate_list_id['A0023']['unterBegriff']['A0023_041']);
// pre_print_r(array_keys($o_u_duplicate_list_id['A0023']['unterBegriff']['A0023_041']['group']));
